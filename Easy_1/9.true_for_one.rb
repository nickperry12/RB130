...
Write a method `one?` that returns `true` if exactly one iteration of the block
returns `true`. If the block returns `true` a second time, or doesn't return
`true` at all, it should return `false`.

A:

/* given an array `arr` */

Initialize `true_counter` to 0
Iterate through the given array
- Yield each object to the block
  - If the block returns `true`, increment `true_counter` by 1
  - Break out of iteration and return false if `true_counter` is greater than 1
- Evaulate if `true_counter` is equal to 1
=end
