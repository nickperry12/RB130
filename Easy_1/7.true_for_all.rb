...
Need to define a method `all?` that functions similarily to the
`Enumerable#all?` method. It should continue iterating until the block either
returns `false`, in which case, the method should return `false`, or upon
finishing iteration it should return `true`
...
While `counter` is less than the size of the given array
- Yield the current element referenced by `arr[counter]` to the block
  - if the block returns a truthy value, increment `counter` by 1
  - if the block returns a falsy value, return `false` and break out of the loop
=end
